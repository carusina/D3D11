...
void D3D11Utils::CreateTexture(
    ...
    // 4채널로 만들어서 복사
    // GPU가 효율적으로 처리하는 데이터 형식인 32비트 RGBA 형식으로 Pixel Data를 재정렬
    ...
    txtDesc.Width = width; // 이미지의 가로 크기
    txtDesc.Height = height; // 이미지의 세로 크기
    txtDesc.MipLevels = txtDesc.ArraySize = 1;
    txtDesc.Format = DXGI_FORMAT_R8G8B8A8_UNORM; // Texture의 Pixel Data 형식을 지정
                                                 // RGBA 각 8비트씩 총 32비트, UNORM은 0 ~ 255 사이의 정수 값 Shader에서 0.0f ~ 1.0f의 실수로 자동 변환
    txtDesc.SampleDesc.Count = 1;
    txtDesc.Usage = D3D11_USAGE_IMMUTABLE; // Texture가 로드되면 변경 X
    txtDesc.BindFlags = D3D11_BIND_SHADER_RESOURCE; // 이 Texture는 Shader에서 읽어가는 Shader Resource로 사용

    // Fill in the subresource data.
    D3D11_SUBRESOURCE_DATA initData;
    initData.pSysMem = image.data();
    initData.SysMemPitch = txtDesc.Width * sizeof(uint8_t) * 4; // Texture의 가로 한 줄의 크기
    // initData.SysMemSlicePitch = 0;

    device->CreateTexture2D(&txtDesc, &initData, texture.GetAddressOf()); // GPU 메모리(VRAM)에 실제 Texture 공간을 할당하고 Pixel Data를 복사
                                                                          // texture 변수가 실제 Texture 객체를 가리킴
    device->CreateShaderResourceView(texture.Get(), nullptr,
                                     textureResourceView.GetAddressOf()); // Shader는 Texture 메모리에 직접 접근이 불가능해서
                                                                          // textureResourceView를 통해 texture에 접근
}
