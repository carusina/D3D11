...
class D3D11Utils {
  public:
    ...
    static void CreateVertexBuffer(ComPtr<ID3D11Device> &device,
        ...
        bufferDesc.Usage = D3D11_USAGE_IMMUTABLE; // Buffer 생성 후 내용이 절대 바뀌지 않음
        bufferDesc.ByteWidth = UINT(sizeof(T_VERTEX) * vertices.size()); // 생성할 버퍼의 크기 = Vertex 하나의 크기 * Vertex 개수
        bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER; // 이 버퍼는 렌더링 파이프라인 IA 단계에서 Vertex Buffer로 Bind
        bufferDesc.CPUAccessFlags = 0; // 생성 이후 CPU가 Write 할 일이 없으므로 0으로 설정
        bufferDesc.StructureByteStride = sizeof(T_VERTEX); // Vertex Buffer 안에서 한 정점에서 다음 정점까지의 메모리 간격
    ...
    static void CreateConstantBuffer(ComPtr<ID3D11Device> &device,
        ...
        cbDesc.ByteWidth = sizeof(constantBufferData); // 생성할 버퍼의 크기
        cbDesc.Usage = D3D11_USAGE_DYNAMIC; // 버퍼의 주요 사용 방식을 지정하는 플래그
                                            // 이 플래그를 보고 DirectX가 CPU와 GPU 사이의 데이터 전송을 최적화
                                            // D3D11_USAGE_DYNAMIC => CPU가 자주 Update하고 GPU는 Read만
        cbDesc.BindFlags = D3D11_BIND_CONSTANT_BUFFER; // 버퍼의 용도
                                                       // D3D11_BIND_CONSTANT_BUFFER => 이 Buffer는 렌더링 파이프라인의 Constant Buffer와 Bind한다.
        cbDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE; // CPU가 이 버퍼에 대해 어떤 작업을 할 수 있는지 권한을 설정
        cbDesc.MiscFlags = 0;
        cbDesc.StructureByteStride = 0;

        // Fill in the subresource data.
        D3D11_SUBRESOURCE_DATA initData;
        initData.pSysMem = &constantBufferData; // 버퍼를 채울 Data
        initData.SysMemPitch = 0;
        initData.SysMemSlicePitch = 0;

        auto hr = device->CreateBuffer(&cbDesc, &initData,
                                       constantBuffer.GetAddressOf()); // constantBuffer.GetAddressOf() => 생성된 버퍼 객체를 저장할 포인터 주소
        if (FAILED(hr)) {
    ...
    static void UpdateBuffer(ComPtr<ID3D11Device> &device,
        ...
        context->Map(buffer.Get(), NULL, D3D11_MAP_WRITE_DISCARD, NULL, &ms); // GPU에 있는 Constant Buffer를 잠그고 CPU가 접근할 수 있는 메모리 주소를 획득
        memcpy(ms.pData, &bufferData, sizeof(bufferData)); // GPU Constant Buffer에 데이터를 복사
        context->Unmap(buffer.Get(), NULL); // Constant Buffer 잠금을 해제
    }
