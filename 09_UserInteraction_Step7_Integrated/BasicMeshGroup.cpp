...
void BasicMeshGroup::Initialize(ComPtr<ID3D11Device> &device,
    ...
    // ConstantBuffer 만들기
    // Constant Buffer: Shader에 전달할 Uniform Data (Matrix, Light...)
    ...
    for (const auto &meshData : meshes) {
        ...
        // 3D 모델의 모든 Vertex 정보를 GPU 메모리에 업로드
        D3D11Utils::CreateVertexBuffer(device, meshData.vertices,
                                       newMesh->vertexBuffer);
        // Vertex Buffer에 있는 Vertex들을 어떤 순서로 연결해서 삼각형을 만들 것인지
        ...
        if (!meshData.textureFilename.empty()) {
            ...
            // 이미지 파일 (.jpg, .png...)을 읽어서 Pixel Shader가 색상 정보로 사용할 수 있는 GPU용 Texture Resource를 생성
            D3D11Utils::CreateTexture(device, meshData.textureFilename,
                                      newMesh->texture,
                                      newMesh->textureResourceView);
        }

        // 각 Mesh는 같이 변환(Scale, Rotate, Translate...)이 되어야 하므로
        // 같은 Constant Buffer를 사용
...
void BasicMeshGroup::UpdateModelWorld(const Matrix &modelWorldRow) {
    // Class 멤버 변수에 새로운 월드 행렬 저장
    this->m_modelWorldRow = modelWorldRow;

    // Normal Vector를 위한 Inverse Transpose 행렬
    this->m_invTransposeRow = modelWorldRow;
    m_invTransposeRow.Translation(Vector3(0.0f)); // Normal Vector는 방향을 나타내는 벡터이기 때문에
                                                  // World 행렬에 포함된 Translation 정보는 제거
    m_invTransposeRow = m_invTransposeRow.Invert().Transpose();

    // Shader의 Constant Buffer로 보낼 행렬 (Transpose 포함)
    // DirectX는 Row-Major이지만 HLSL 셰이더는 Column-Major라서 Transpose
